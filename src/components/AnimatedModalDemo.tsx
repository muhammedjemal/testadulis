...
import Image from "next/image";
import { motion } from "framer-motion";
import FixedRightButton from "@/components/FixedRightButton";
...
export function AnimatedModalDemo() {
  ...
  const [modalOpen, setModalOpen] = useState(false); // Global State

  return (
    <div className="relative flex items-center justify-center">
      <FixedRightButton onClick={() => setModalOpen(true)} />
      <Modal>
        <ModalBody open={modalOpen} setOpen={setModalOpen}>
          {" "}
          {/* Pass the state and setter */}
          {/* @ts-ignore */}
          <ModalContent>
            <h4 className="text-lg md:text-2xl text-neutral-600 dark:text-neutral-100 font-bold text-center mb-8">
              Own a Piece of
            ...
            <div className="py-10 flex flex-wrap gap-x-4 gap-y-6 items-start justify-start max-w-sm mx-auto">
              <div className="flex  items-center justify-center">
                <span className="text-neutral-700 dark:text-neutral-300 text-sm w-full">
                  Own a numbered, limited edition Artworks from Ethiopia
                  {"'"}s most talented and handpicked artists. Each print is
                  numbered and is one of only 100, making your selection truly
                  unique. Subscribe now for 20% off Christmas discount, receive
                  a special welcome gift and be one of 100 owners of these
                  unique works.
                </span>
              </div>
              <div className="relative w-full max-w-sm">
